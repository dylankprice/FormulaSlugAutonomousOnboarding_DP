...
X=torch.randint(0,10, (100, 2)).float().to(device) #creates data range of int pair
y =X[:, 0] + X[:, 1]#creates output by adding the pairs
...
class LinearRegressionModelIV2(nn.Module):
    def __init__(self):
        super().__init__()
        self.linear_layer = nn.Linear(in_features=2, out_features=2)
        self.linear_layer2 = nn.Linear(in_features=2, out_features=1)
